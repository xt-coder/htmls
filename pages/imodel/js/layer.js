...
;void function(window, undefined){		
	var pathType = false, //是否采用自动获取绝对路径。 false：将采用相对路径，需配置layerPath
	pathUrl = 'http://www.imodel.cc/static/js/layer/', //相对路径url
	ready = {
		getPath : function(){
	...
	CLASS.prototype = {
		type : function(){
			...
			var times = ready.global.times, newconfig = ready.global.config[times] = this.config;
			newconfig.dialog = this.dialog;
			newconfig.page = this.page;
			newconfig.iframe = this.iframe;
			newconfig.loading = this.loading;
			newconfig.tips = this.tips;
